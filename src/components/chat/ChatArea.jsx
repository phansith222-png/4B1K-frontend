...
const NoScrollbarStyle = React.memo(() => <style>{SCROLLBAR_CSS}</style>);
NoScrollbarStyle.displayName = "NoScrollbarStyle";
  ...
  return (
    <header className="shrink-0 z-[100] border-b border-white/5 bg-[#0B0C10]/80 backdrop-blur-xl relative">
      ...
      <div className="flex items-center gap-4 px-4 md:px-6 py-2.5 md:py-3 relative z-10">
        ...
        <motion.div
          layoutId={`avatar-${activeRoom?._id}`}
            ...
            {isEditingName ? (
              <input
                ...
                onKeyDown={(e) => {
                  ...
                  if (e.key === "Escape") { setEditName(roomName); setIsEditingName(false); }
                }}
                  ...
                  <>
                    ...
                    >
                      {isGroup && activeRoom?.creatorId === myUserId && (
                        <>
                          <button
                            onClick={() => { setShowGroupMenu(false); fileInputRef.current?.click(); }}
                          ...
                          <button
                            onClick={() => { setShowGroupMenu(false); setEditName(roomName); setIsEditingName(true); }}
                            className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-gray-300 hover:text-white hover:bg-white/5 transition-all text-left"
                          >
                      ...
                      <button
                        onClick={() => { setShowGroupMenu(false); handleDeleteRoom(); }}
...
const ChatFooter = React.memo(({
  ...
  const handleEmojiSelect = (em) => {
    ...
    setInputText(inputText.slice(0, start) + em + inputText.slice(end));
    setShowEmoji(false);
