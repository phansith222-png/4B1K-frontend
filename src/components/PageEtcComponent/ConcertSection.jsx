        ...
        <section className="relative w-full py-24 px-6 bg-[#0a0a0a] border-y border-gray-900">
            <div className="max-w-7xl mx-auto relative z-10">
                ...
                <div className="flex flex-col md:flex-row justify-between items-end mb-14">
                    ...
                    <button className="flex items-center gap-2 text-xs font-bold text-[#CEFF67] hover:text-white uppercase tracking-widest transition-colors mt-4 md:mt-0 group">
                        VIEW ALL SHOWS
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
                    {events.slice(0, 4).map((item, idx) => {
                        ...
                        return (
                        <div key={idx} className="relative rounded-[2rem] overflow-hidden group bg-[#111] shadow-[0_10px_30px_rgba(0,0,0,0.5)] border border-gray-800 hover:border-[#2B5AE8]/50 h-[400px] cursor-pointer transition-colors duration-500">
                            <img
                            ...
                            <div className="absolute inset-0 p-8 flex flex-col justify-end z-20">
                                <span className="bg-[#CEFF67] text-black w-fit px-3 py-1 rounded-full text-[10px] font-black mb-3 tracking-widest uppercase shadow-[0_0_10px_#CEFF67]">
                                    {new Date(evt.startTime).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                </span>
                                <h4 className="text-2xl font-black text-white line-clamp-2 group-hover:text-[#2B5AE8] transition-colors">{evt.eventName}</h4>
