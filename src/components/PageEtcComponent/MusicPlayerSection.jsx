    ...
    return (
        <section className="relative w-full py-24 px-6 bg-transparent overflow-hidden">
            <div className="absolute inset-0 opacity-10 pointer-events-none">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[800px] bg-gradient-to-r from-[#7000FF] via-transparent to-[#00E5FF] rounded-[100%] blur-[80px]" style={{ willChange: 'transform, opacity' }}></div>
            </div>

            <div className="relative z-10 max-w-7xl mx-auto px-2 md:px-6">
                <Reveal effect="fade-up">
                    <h3 className="text-center text-xl md:text-2xl font-black tracking-[0.5em] uppercase mb-12 text-[#00E5FF]">Top Chart</h3>
                </Reveal>

                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-5 mb-20">
                    {songs.slice(0, 5).map((item, idx) => (
                        <Reveal key={idx} delay={idx * 0.1} effect="fade-up">
                            <button
                                onClick={(e) => handleSongSelect(idx, e)}
                                className={`border ${currentSongIndex === idx ? 'border-[#00E5FF]' : 'border-gray-800'} bg-[#111] hover:border-[#00E5FF] text-white rounded-3xl p-6 transition-all duration-300 w-full group flex items-center gap-5 shadow-lg shadow-black/50 text-left hover:bg-[#1a1a1a]`}
                            >
                                <div className={`flex-shrink-0 w-12 h-12 rounded-2xl ${currentSongIndex === idx ? 'bg-[#00E5FF] text-white' : 'bg-[#00E5FF]/20 text-[#00E5FF]'} flex items-center justify-center font-black text-2xl group-hover:bg-[#00E5FF] group-hover:text-white transition-colors`}>
                                    {currentSongIndex === idx && isPlaying ? (
                                        <div className="flex gap-0.5 items-end h-4">
                                            <div className="w-1 bg-[#7000FF] eq-bar" style={{ animationDelay: '0.1s' }}></div>
                                            <div className="w-1 bg-[#7000FF] eq-bar" style={{ animationDelay: '0.2s' }}></div>
                                            <div className="w-1 bg-[#7000FF] eq-bar" style={{ animationDelay: '0.3s' }}></div>
                                        </div>
                                    ) : (
                                        idx + 1
                                    )}
                        ...
                        <div className="w-full max-w-5xl mx-auto bg-[#111111]/80 backdrop-blur-3xl rounded-[2.5rem] p-6 md:p-8 lg:p-10 flex flex-col lg:flex-row items-center justify-between gap-8 md:gap-10 border border-white/5 shadow-[0_30px_60px_rgba(0,0,0,0.8)] relative overflow-hidden">
                            ...
                            <div className="relative w-32 h-32 md:w-40 md:h-40 flex-shrink-0 flex items-center justify-center">
                                ...
                                <div className="w-full h-full rounded-full bg-[#111111] border-[4px] border-[#222] cd-rotate flex items-center justify-center shadow-[0_0_30px_rgba(0,0,0,0.9)] relative z-10 overflow-hidden ring-2 ring-[#00E5FF]/20" style={{ animationPlayState: isPlaying ? 'running' : 'paused' }}>
                                    <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-black border-2 border-gray-700 shadow-inner relative flex items-center justify-center z-20">
                                        <div className="w-2 h-2 rounded-full bg-[#7000FF]"></div>
                                    </div>
                            ...
                            <div className="flex-1 w-full flex flex-col justify-center px-2 md:px-6 relative z-10">
                                <div className="flex flex-col items-center lg:items-start mb-6">
                                    ...
                                    <p className="text-[#7000FF] text-sm md:text-base font-black tracking-widest uppercase mt-1 text-center lg:text-left">
                                        {artist.artistName}
                                    </p>
                                </div>
                            ...
                            <div className="flex items-center gap-4 md:gap-6 mt-4 lg:mt-0 relative z-10">
                                ...
                                <button
                                    ...
                                    className="relative flex-shrink-0 flex items-center justify-center w-16 h-16 md:w-20 md:h-20 rounded-full bg-gradient-to-br from-[#00E5FF] via-[#7000FF] to-[#00E5FF] text-white transition-all duration-300 ease-out hover:scale-105 active:scale-95 group"
                                >
                                    <div className="absolute inset-0 rounded-full bg-gradient-to-br from-[#00E5FF] to-[#7000FF] blur-[20px] opacity-50 group-hover:opacity-80 transition-opacity duration-300 z-0"></div>
