    ...
    return (
        <section className="relative w-full py-24 px-6 overflow-hidden">
            <div className="absolute inset-0 opacity-10 pointer-events-none">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[800px] bg-gradient-to-r from-[#2B5AE8] via-transparent to-[#CEFF67] rounded-[100%] blur-[80px] animate-pulse" style={{ willChange: 'transform, opacity' }}></div>
            </div>
                    ...
                    {songs.slice(0, 4).map((item, idx) => (
                        <Reveal key={idx} delay={idx * 0.1} effect="fade-up">
                            <button 
                                ...
                                className={`border ${currentSongIndex === idx ? 'border-[#2B5AE8]' : 'border-gray-800'} bg-[#111] hover:border-[#2B5AE8] text-white rounded-3xl p-6 transition-all duration-300 w-full group flex items-center gap-5 shadow-lg shadow-black/50 text-left hover:bg-[#1a1a1a]`}
                            >
