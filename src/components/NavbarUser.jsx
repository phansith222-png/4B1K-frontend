...
export default function NavbarUser({ isLanding = false }) {
    ...
    // จัดการการปิดเมนูเมื่อคลิกที่อื่น
    useEffect(() => {
    ...
    return (
        <div className="relative">
            <style>{`
                ...
                /* กรอบรูป Profile สีวิ่งช้าๆ */
                .profile-gradient-border {
                    background: linear-gradient(#1A1C23, #1A1C23) padding-box,
                                linear-gradient(90deg, #00E5FF, #7000FF, #FF00FF, #00E5FF) border-box;
                    background-size: 300% auto;
                    border: 2px solid transparent;
                    border-radius: 50%;
                    animation: runGradient 5s linear infinite; /* วิ่งช้าๆ 5 วินาที */
                }

                /* ปุ่ม Login / Register แบบหน้า Landing */
                .btn-custom-login {
                    ...
                    border-radius: 12px;
                    transition: all 0.3s;
                    width: 105px; height: 38px; display: flex; align-items: center; justify-content: center;
                }
                .btn-custom-login:hover { box-shadow: 0 4px 15px rgba(0, 229, 255, 0.25); color: #00E5FF; }

                .btn-custom-register {
                    background: linear-gradient(90deg, #00E5FF, #7000FF, #00E5FF);
                    background-size: 200% auto;
                    animation: runGradient 3s linear infinite;
                    border: none; color: #FFFFFF; border-radius: 12px; font-weight: 800;
                    transition: all 0.3s; width: 105px; height: 38px; display: flex; align-items: center; justify-content: center;
                    box-shadow: 0 4px 15px rgba(112, 0, 255, 0.3);
                }
                .btn-custom-register:hover { box-shadow: 0 6px 20px rgba(0, 229, 255, 0.5); }
                    ...
                    {isLanding ? (
                        ...
                        <>
                            <button onClick={() => navigate('/chat')} className="btn-custom-chat hidden sm:flex">
                                <span className="text-gradient-chat text-[13px] font-extrabold tracking-widest uppercase">Chat</span>
                                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 text-[#00E5FF]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                                </svg>
                            </button>

                            <div className="relative" ref={profileRef}>
                                {/* เปลี่ยนขอบ Profile ตรงนี้ */}
                                <button onClick={() => setIsProfileMenuOpen(!isProfileMenuOpen)} className="flex items-center gap-2 focus:outline-none transition-all hover:opacity-80">
                                    ...
                                    <svg className={`w-3.5 h-3.5 text-gray-400 transition-transform duration-300 ${isProfileMenuOpen ? 'rotate-180 text-[#00E5FF]' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M19 9l-7 7-7-7" />
                                    </svg>
                                </button>
                                ...
                                <AnimatePresence>
                                    {isProfileMenuOpen && (
                                        <motion.div 
                                            ...
                                            className="absolute right-0 mt-4 w-56 bg-[#1A1C23] rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 py-2 z-50"
                                        >
                                            <div className="px-4 py-3 border-b border-white/5 mb-1">
                                                <p className="text-sm font-bold text-white truncate">John Doe</p>
                                                <p className="text-xs font-medium text-gray-400 truncate">john.doe@example.com</p>
                                            </div>
                                            <div className="flex flex-col px-1.5">
                                                <button onClick={() => handleNavigate('/editprofile')} className="flex items-center gap-3 px-3 py-2.5 text-sm font-medium text-gray-300 hover:text-[#00E5FF] hover:bg-[#252830] rounded-xl transition-colors text-left">
            ...
            <AnimatePresence>
                {isArtistMenuOpen && (
                    <motion.div 
                        ...
                        className="absolute top-full left-0 right-0 w-full z-40 bg-[#0B0C10]/95 backdrop-blur-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] rounded-b-3xl border-t border-white/10 pb-12 pt-8" 
                        ref={menuRef}
                    >
                                        ...
                                        {chartOrder.map((chartIndex, position) => {
                                            ...
                                            return (
                                                <div 
                                                    ...
                                                    style={{ top: `${position * 95}px`, zIndex: 10 - position }}
                                                >
                                                    <div className="w-[85px] h-[65px] rounded-xl overflow-hidden shadow-sm shrink-0 relative">
                                                        <div className="absolute inset-0 bg-black/40 group-hover:bg-transparent transition-colors z-10"></div>
                                                        <img src={`https://images.unsplash.com/photo-${item.img}?q=80&w=200&auto=format&fit=crop`} alt="Chart Item" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                                                    </div>
