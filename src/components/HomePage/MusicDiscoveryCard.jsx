...
import { getAllArtists, getSongsByArtist } from '../../api/artist';
...
export default function MusicDiscoveryCard() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        else if (Array.isArray(resp?.artists)) list = resp.artists;
        else if (Array.isArray(resp?.data?.artists)) list = resp.data.artists;

        console.log(`[MusicCard] STAGE 1 SUCCESS: Found ${list.length} artists`);

        if (list && list.length > 0) {
          setArtists(list);
          // Pick artist immediately
          const randomIdx = Math.floor(Math.random() * list.length);
          const firstArtist = list[randomIdx];
          setCurrentArtist(firstArtist);
          console.log(`[MusicCard] STAGE 1: Selected Artist = ${firstArtist.artistName}`);
          
          // Now fetch songs for this specific artist
          await fetchSongsForArtist(firstArtist.id, firstArtist.artistName);
        } else {
          setLoading(false);
  ...
  const fetchSongsForArtist = async (artistId, artistName) => {
    try {
      console.log(`[MusicCard] STAGE 2: Fetching songs for ${artistName} (ID: ${artistId})...`);
      const songResp = await getSongsByArtist(artistId);
      
      let songList = [];
      const rawData = songResp?.data || songResp;
      if (Array.isArray(rawData)) songList = rawData;
      else if (Array.isArray(rawData?.songs)) songList = rawData.songs;
      else if (Array.isArray(rawData?.data)) songList = rawData.data;

      if (songList.length > 0) {
        console.log(`[MusicCard] STAGE 2 SUCCESS: Found ${songList.length} songs for ${artistName}. Match confirmed.`);
        setSongs(songList);
        setLoading(false);
        setCountdown(15);
      } else {
        console.warn(`[MusicCard] STAGE 2: ${artistName} has 0 songs. Searching for another artist...`);
        // If no songs, pick another one automatically
  ...
  const pickRandomArtist = async () => {
    ...
    // Select a random artist that isn't the current one (if possible)
    ...
    setSongs([]); // Clear old songs immediately
    await fetchSongsForArtist(randomArtist.id, randomArtist.artistName);
  };
  ...
  const currentSong = songs[0]; // Always show the first song for simplicity in shuffle
