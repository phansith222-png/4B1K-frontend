                                ...
                                return chartData.length > 0 ? chartData.map((data, idx) => (
                                    <div key={idx} className="flex-1 flex flex-col items-center h-full justify-end group relative cursor-crosshair z-10">
                                        <div className="flex w-full justify-center gap-1.5 md:gap-3 items-end h-full px-1">
                                            <motion.div initial={{ height: 0 }} whileInView={{ height: `${data.youtubePop}%` }} className="w-[45%] md:w-6 bg-[#FF0000] rounded-t-md transition-all duration-500 group-hover:brightness-125 shadow-[0_0_15px_rgba(255,0,0,0.3)]" />
                                            <motion.div initial={{ height: 0 }} whileInView={{ height: `${data.spotifyPop}%` }} className="w-[45%] md:w-6 bg-[#1DB954] rounded-t-md transition-all duration-500 group-hover:brightness-125 shadow-[0_0_15px_rgba(29,185,84,0.3)] relative" />
                                        </div>
                                        <span className="font-sub text-[#b266c5] text-[10px] md:text-xs font-bold uppercase mt-6 truncate w-full text-center px-1 group-hover:text-[#f9c1db] transition-colors">{data.song.title.split(' ')[0]}</span>
                                        ...
                                        <div
                                            ...
                                            style={{ bottom: `calc(${data.maxHeight}% + 55px)` }}
                                        >
                                            <div className="bg-[#1c172e]/95 backdrop-blur-md border border-[#d83bb6]/50 p-4 rounded-xl shadow-[0_10px_30px_rgba(155,45,150,0.5)] min-w-[150px] text-center">
                                                <p className="text-[11px] font-classic font-bold text-white border-b border-[#b266c5]/30 pb-2 mb-2 truncate">{data.song.title}</p>
                                                <div className="flex justify-between items-center text-[10px] font-sub tracking-widest font-bold">
                                                    <span className="text-[#FF0000]">YT: <span className="text-white">{data.ytValue}M</span></span>
                                                    <span className="text-[#1DB954]">SP: <span className="text-white">{data.spValue}M</span></span>
                                                </div>
                                            </div>
