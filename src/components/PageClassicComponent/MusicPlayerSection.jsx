    ...
    return (
        <section className="relative w-full py-32 px-6 overflow-hidden z-10">
            <div className="absolute inset-0 opacity-[0.15] pointer-events-none">
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[100%] h-[800px] bg-gradient-to-r from-[#9b2d96] via-transparent to-[#b266c5] rounded-[100%] blur-[80px] animate-pulse" style={{ willChange: 'transform, opacity' }}></div>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-28">
                    {songs.slice(0, 4).map((item, idx) => (
                        <Reveal key={idx} delay={idx * 0.1} effect="fade-up">
                            ...
                            >
                                <div className={`w-8 flex justify-center transition-colors ${currentSongIndex === idx && isPlaying ? 'text-[#d83bb6] drop-shadow-[0_0_10px_#d83bb6]' : 'text-[#b266c5] group-hover:text-[#f9c1db]'}`}>
                                    {currentSongIndex === idx && isPlaying ? (
                                        <div className="flex gap-1 items-end h-6 pb-1">
                                            <div className="w-1.5 bg-[#d83bb6] eq-bar rounded-t-sm" style={{ animationDelay: '0.1s' }}></div>
                                            <div className="w-1.5 bg-[#d83bb6] eq-bar rounded-t-sm" style={{ animationDelay: '0.2s' }}></div>
                                            <div className="w-1.5 bg-[#d83bb6] eq-bar rounded-t-sm" style={{ animationDelay: '0.3s' }}></div>
                                        </div>
                                    ) : (
                                        <span className="font-classic text-2xl md:text-3xl italic">0{idx + 1}</span>
                                    )}
                                </div>
                                <div>
                                    ...
                                    <p className="font-sub text-[#b266c5] mt-2 tracking-widest text-xs md:text-sm font-semibold">
                ...
                <Reveal delay={0.2} effect="fade-up">
                    <div className="w-full max-w-5xl mx-auto bg-[#1c172e]/90 backdrop-blur-3xl rounded-[2.5rem] p-6 md:p-8 lg:p-10 flex flex-col lg:flex-row items-center justify-between gap-8 md:gap-10 border border-[#b266c5]/20 shadow-[0_30px_60px_rgba(48,41,78,0.9)] relative overflow-hidden">
                        ...
                        <div className="relative w-32 h-32 md:w-40 md:h-40 flex-shrink-0 flex items-center justify-center">
                            <div className="absolute inset-0 flex items-end justify-center gap-2 opacity-40 z-0 overflow-hidden">
                                {[...Array(8)].map((_, i) => (
                                    <div 
                                        key={i} 
                                        className="w-full bg-gradient-to-t from-[#9b2d96] to-[#f9c1db] rounded-t-full eq-bar" 
                        ...
                        <div className="flex-1 w-full flex flex-col justify-center px-2 md:px-6 relative z-10">
                            ...
                            <div className="w-full mt-2">
                                <div className="w-full bg-[#30294e]/60 rounded-full h-1.5 md:h-2 relative cursor-pointer group shadow-inner border border-white/5" onClick={handleProgressClick}>
                                    <motion.div 
                                ...
                                <div className="flex justify-between font-sub text-[10px] md:text-xs text-[#b266c5] mt-3 tracking-widest font-bold">
