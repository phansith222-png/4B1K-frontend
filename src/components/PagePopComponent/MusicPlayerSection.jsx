    ...
    return (
        <section className="relative w-full py-32 px-6 overflow-hidden z-10">
            ...
            <div className="relative z-10 max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-24">
                    {songs.slice(0, 4).map((item, idx) => (
                        <button 
                            ...
                            className={`border ${currentSongIndex === idx ? 'border-[#00F5D4] shadow-[0_0_20px_rgba(0,245,212,0.15)] bg-[#1A1C23]/90' : 'border-white/5 shadow-lg bg-[#1A1C23]/40'} hover:border-[#00F5D4]/50 hover:bg-[#1A1C23]/90 text-white rounded-[2rem] p-6 md:p-8 transition-all duration-300 group flex items-center gap-6 backdrop-blur-xl text-left`}
                        >
                            ...
                            <div className="flex-1 flex flex-col gap-1.5">
                                ...
                                <span className="text-xs text-gray-400 font-bold tracking-widest uppercase">
                                    {item.popularity ? `${(item.popularity / 1000000).toFixed(1)}M Streams` : 'Trending'}
                                </span>
                            </div>
                        </button>
                    ))}
                ...
                {/* ================= MUSIC PLAYER CONTROLS (UI อัปเกรด) ================= */}
                ...
                <div className="w-full max-w-5xl mx-auto bg-[#181a20]/80 backdrop-blur-3xl rounded-[2.5rem] p-6 md:p-8 lg:p-10 flex flex-col lg:flex-row items-center justify-between gap-8 md:gap-10 border border-white/5 shadow-[0_30px_60px_rgba(0,0,0,0.7)] relative overflow-hidden">
                    ...
                    <div className="relative w-32 h-32 md:w-40 md:h-40 flex-shrink-0 flex items-center justify-center">
                        <div className="absolute inset-0 flex items-end justify-center gap-2 opacity-30 z-0 overflow-hidden">
                            {[...Array(8)].map((_, i) => (
                                <div 
                                    key={i} 
                                    className="w-full bg-gradient-to-t from-[#FF007F] to-[#00F5D4] rounded-t-full eq-bar" 
                    ...
                    <div className="flex-1 w-full flex flex-col justify-center px-2 md:px-6 relative z-10">
                        ...
                        <div className="w-full mt-2">
                            <div className="w-full bg-white/5 rounded-full h-1.5 md:h-2 relative cursor-pointer group" onClick={handleProgressClick}>
                                <motion.div 
                    ...
                    {/* ส่วนขวา: ปุ่มควบคุม (Soft & Natural Buttons) */}
                    <div className="flex items-center gap-4 md:gap-6 mt-4 lg:mt-0 relative z-10">
                        ...
                        {/* Play/Pause Button (Beautiful Soft Glow) */}
                        <button 
