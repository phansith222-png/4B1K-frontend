 ...
 */
export function InputField({ type = 'text', placeholder, registration, error, className = '', ...props }) {
  return (
    <div className={`relative ${className}`}>
      ...
      />
      {error && (
        <p className="text-red-400 text-xs font-bold mt-1 pl-2 tracking-tight">{error.message}</p>
      )}
    </div>
  )
}
