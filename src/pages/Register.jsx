...
export default function Register() {
  ...
  const { register, handleSubmit, formState, reset } = useForm({
    ...
    defaultValues: {
      email: '',      // <-- ใช้ email 
      telephone: '',  // <-- และ telephone แยกกัน
      username: '',
      password: '',
      confirmPassword: '',
      firstName: '',
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      const payload = {
        ...
        email: values.email, // <-- ส่ง Email เสมอ (Prisma จะไม่ Error แล้ว)
        telephone: values.telephone || undefined, // ถ้าไม่ได้กรอกเบอร์ ให้ส่งเป็น undefined
      ...
      <main className="flex-1 flex flex-col lg:flex-row w-full max-w-[1440px] mx-auto relative z-10">
        ...
        <div className="hidden lg:flex w-1/2 relative overflow-hidden bg-[#F9F8F3]">
          ...
          <div className="relative z-10 w-full p-12 xl:p-20 flex flex-col justify-center h-full">
            ...
            <p className="text-lg text-gray-500 leading-relaxed font-medium">
              Dive into the ultimate hub for <span className="text-[#2B5AE8] font-bold">concert lovers</span>.
            </p>
            <div className="mt-10 flex items-center gap-4">
              <div className="flex -space-x-3">
                ...
                <div className="w-11 h-11 rounded-full border-[3px] border-[#F9F8F3] bg-[#CEFF67] shadow-sm flex items-center justify-center text-gray-900 font-bold text-[10px]">10k+</div>
              </div>
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center p-6 lg:p-12 xl:p-20 relative">
          <div className="w-full max-w-[420px]">
            <h1 className="text-[40px] font-black text-gray-900 mb-2 tracking-tight">Sign Up</h1>
            <p className="text-gray-500 mb-8 font-medium">Please enter your details to create an account.</p>

            {success && <div className="mb-6 py-3 px-4 rounded-2xl bg-green-50 text-green-600 border border-green-200 text-sm flex items-center gap-2 shadow-sm">✅ สมัครสมาชิกสำเร็จ! กำลังพาไปหน้า Login...</div>}
            {apiError && <div className="mb-6 py-3 px-4 rounded-2xl bg-red-50 text-red-600 border border-red-100 text-sm flex items-center gap-2 shadow-sm">⚠️ {apiError}</div>}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4" noValidate>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="relative">
                  <input type="text" placeholder="Firstname *" {...register('firstName')}
                    className="w-full bg-white px-5 py-4 rounded-2xl border border-gray-200 focus:border-[#2B5AE8] outline-none text-sm transition-all shadow-sm"
                    style={{ borderColor: errors.firstName ? '#ef4444' : '' }} />
                  {errors.firstName && <p className="text-red-500 text-[10px] mt-1 pl-2 absolute -bottom-5">{errors.firstName.message}</p>}
                </div>
                <div className="relative">
                  <input type="text" placeholder="Lastname *" {...register('lastName')}
                    className="w-full bg-white px-5 py-4 rounded-2xl border border-gray-200 focus:border-[#2B5AE8] outline-none text-sm transition-all shadow-sm"
                    style={{ borderColor: errors.lastName ? '#ef4444' : '' }} />
                  {errors.lastName && <p className="text-red-500 text-[10px] mt-1 pl-2 absolute -bottom-5">{errors.lastName.message}</p>}
                </div>
              </div>
              ...
              {/* แยกช่อง Email ออกมาแบบเจาะจง */}
              <div className="relative mt-2">
                <input type="email" placeholder="Email address *" {...register('email')}
                  className="w-full bg-white px-5 py-4 rounded-2xl border border-gray-200 focus:border-[#2B5AE8] outline-none text-sm transition-all shadow-sm"
                  style={{ borderColor: errors.email ? '#ef4444' : '' }} />
                {errors.email && <p className="text-red-500 text-[10px] mt-1 pl-2 absolute -bottom-5">{errors.email.message}</p>}
              </div>

              {/* ช่องเบอร์โทร (Optional) */}
              <div className="relative mt-2">
                <input type="tel" placeholder="Telephone (Optional)" {...register('telephone')}
                  className="w-full bg-white px-5 py-4 rounded-2xl border border-gray-200 focus:border-[#2B5AE8] outline-none text-sm transition-all shadow-sm"
                  style={{ borderColor: errors.telephone ? '#ef4444' : '' }} />
                {errors.telephone && <p className="text-red-500 text-[10px] mt-1 pl-2 absolute -bottom-5">{errors.telephone.message}</p>}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
                ...
                <div className="relative">
                  ...
                  {errors.confirmPassword && <p className="text-red-500 text-[10px] mt-1 pl-2 absolute -bottom-5">{errors.confirmPassword.message}</p>}
                </div>
              </div>

              <button type="submit" disabled={isSubmitting}
                className="w-full py-4 rounded-2xl mt-5 text-[#1b1f27] font-black text-[15px] transition-all hover:-translate-y-1 shadow-lg disabled:opacity-70 flex justify-center items-center"
                style={{ backgroundColor: '#CEFF67' }}>
                {isSubmitting ? <span className="animate-spin inline-block w-5 h-5 border-2 border-gray-900/30 border-t-gray-900 rounded-full" /> : 'Sign Up'}
              </button>
              ...
              <div className="flex justify-center gap-5">
                {['google', 'facebook', 'twitter'].map(provider => (
                  <button key={provider} type="button" onClick={() => handleSocialLogin(provider)} 
                    ...
                    <span className="capitalize text-[10px] font-bold text-gray-400">{provider[0]}</span>
                  </button>
                ))}
              </div>
